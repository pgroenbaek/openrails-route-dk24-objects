...
if __name__ == "__main__":
    ...
    shape_processed_path = "./Processed/DB2Emb"
    ffeditc_path = "./ffeditc_unicode.exe"
    match_shapes = ["DB2f_A1tPnt7_5dRgt.s"]#"DB2f_*Pnt*", "DB2f_*Frog*", "DB2f_*Xover*"]
    ...
    for idx, sfile_name in enumerate(shape_names):
        ...
        # Process .s file
        new_sfile_name = sfile_name.replace("DB2f_", "DB2fb_")
        new_sfile_name = new_sfile_name.replace("DB2_", "DB2b_")
        ...
        tsection_sfile_name = sfile_name.replace("DB2f_", "")
        tsection_sfile_name = tsection_sfile_name.replace("DB2_", "")
        ...
        lod_dlevels = [200, 500]
        mt_trackbase = new_sfile.get_prim_states_by_name("MB_Track2sw")
        rails = new_sfile.get_prim_states_by_name("Rails")[0]
        for lod_dlevel in lod_dlevels:
            mb_track2sw_vertices = new_sfile.get_vertices_by_prim_state(lod_dlevel, mt_trackbase[0])

            trackbed_outer_vertices_left = []
            trackbed_outer_vertices_right = []
            ...
            for mb_track2sw_vertex in mb_track2sw_vertices:
                ...
                if mb_track2sw_vertex.uv_point.u == 0.8620:
                    trackbed_outer_vertices_left.append(mb_track2sw_vertex)
                
                if mb_track2sw_vertex.uv_point.u == -0.1389:
                    trackbed_outer_vertices_right.append(mb_track2sw_vertex)
                
                # TODO Order vertices
                # TODO actually make the embankments
        
            print(trackbed_outer_vertices_left)
